...
begin
  ...
  //RegisterComponents('Data ExControls', [TGIFImage]);
...
begin
  ...
  begin
    //FEvent(self); //原来只有这一句
    P := @FEvent;
    if FImage.FEventRaiseTimer <> nil then
    if (P = @FOnStartPaint) or (P = @FOnPaint) or (P = @FOnAfterPaint) or
      (P = @FOnLoop) or (P = @FOnEndPaint) then
      FImage.FEventRaiseTimer.Enabled := True;
  end;
end;
...
procedure TGIFPainter.OnEventRaiseTimer(Sender: TObject);
begin
